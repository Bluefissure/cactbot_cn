...
class Fisher {
  constructor(element) {
    ...
    this.regex = {
      ...
      'cn': {
        'cast': /00:08c3:(?:[\u4e00-\u9fa5]+)在([\u4e00-\u9fa5]+)甩出了鱼线开始钓鱼。/,
        'bite': /00:08c3:有鱼上钩了！/,
        'catch': /00:0843:([\u4e00-\u9fa5]+)?成功钓上了.+（\d+\.\d星寸）。/,
        'nocatch': /00:08c3:([\u4e00-\u9fa5]+)?(?:不经意间鱼饵被吃掉了……|不经意间丢掉了.+……|不经意间鱼饵被吃掉了……|上钩的鱼逃走了……|鱼线断了！|没有钓到任何东西……\n\n现在使用的鱼饵可能不太适合这片钓场。|没有钓到任何东西……|.+收竿停止了钓鱼。|鱼带着.+逃走了……|这里的鱼现在警惕性很高，看来还是换个地点比较好。|无法持有更多的.+，(?:[\u4e00-\u9fa5]+)?将刚钓上的东西放生了。)/,
        'mooch': /00:08c3:(?:[\u4e00-\u9fa5]+)开始利用上钩的.+尝试以小钓大。/,
        'chumgain': /00:08ae:(?:[\u4e00-\u9fa5]+)附加了“.+撒饵.+”效果。/,
        'chumfade': /00:08b0:(?:[\u4e00-\u9fa5]+)的“.+撒饵.+”状态效果消失了。/,
        'snaggain': /00:08ae:(⇒ )?(?:[\u4e00-\u9fa5]+)附加了“.+钓组.+”效果。/,
        'snagfade': /00:08b0:(?:[\u4e00-\u9fa5]+)的“.+钓组.+”状态效果消失了。/,
        'quit': /00:08c3:(?:[\u4e00-\u9fa5]+)?(?:收回了鱼线。|陷入了战斗不能状态，钓鱼中断。|收回了鱼线。|受到了敌人的攻击，钓鱼中断。)/,
      },
    };
